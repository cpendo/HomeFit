...
const WorkoutCard = ({ workout }) => {
  const { id, name, category, difficulty_level } = workout;
    ...
    <div className="bg-gray-300 w-70 p-5 card">
      <Link to={`/training/workout/${id}`}>
        ...
        <div className="text-sm tracking-tight flex flex-row mt-2 gap-2 ">
          <p className="bg-white px-2 rounded-2xl ">{difficulty_level}</p>
          <p className="bg-white px-2 rounded-2xl ">{categoryName}</p>
        </div>
      </Link>
    </div>
  );
